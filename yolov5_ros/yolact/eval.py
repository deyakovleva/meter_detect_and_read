...
def parse_args(argv=None):
    ...
    parser.add_argument('--trained_model',
                        default='weights/yolact_base_133_3995_interrupt.pth', type=str,
...
def prep_display(dets_out, img, h, w, undo_transform=True, class_color=False, mask_alpha=0.45, fps_str=''):
    ...
    if args.display_masks and cfg.eval_mask_branch and num_dets_to_consider > 0:
        ...
        # get mask coordinates of first detection
        mask1 = masks[0,:,:,None].float()      
        mask__1 = mask1.cpu().numpy()        

        indices_1 = np.where(mask__1[:,:,:]==1)
          
        x_1 = indices_1[0][:]
        y_1 = indices_1[1][:]

        x_1chanel = x_1[::3]
        x_red_1 = x_1chanel[::10000]

        y_1chanel = y_1[::3]
        y_red_1 = y_1chanel[::10000]

        # with open(f'/home/diana/yolact_ws/masks_x_reduced_mano.txt', 'w') as file:
        #     # print('write to file')
        #     for j in range(len(x_1chanel)):
        #         file.write(str(x_1chanel[j]) + ' ' )

        # with open(f'/home/diana/yolact_ws/masks_y_reduced_mano.txt', 'w') as file:
        ...
        # get mask coordinates of second detection
        mask2 = masks[1,:,:,None].float()
        mask__2 = mask2.cpu().numpy()
            
        indices_2 = np.where(mask__2[:,:,:]==1)
...
def prep_coco_cats():
    ...
    print('Prepairing coco cats ready')
...
def evaluate(net:Yolact, dataset, train_mode=False):
    ...
    if args.image is not None:
        ...
        # image_filename = image_path.split("/")[10]
        # only_filename = image_filename.split(".")[0]
